...
void Problem::recurHillClimb(vector<string>& vs, int l, int start, int end)
{
	if (start==end) return;			// base case, if one string
	
	else
	{
		int mid = (start+end)/2;
		
		recurHillClimb(vs,l,start,mid);
		recurHillClimb(vs,l,mid+1,end);
	}
