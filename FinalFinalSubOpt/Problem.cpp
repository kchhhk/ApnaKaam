...
void Problem::initialiseHillClimb(vector<string>& vs, int l, int start, int end)
{
	for (int i = start ; i<end ; i++)
		for (int j = 0 ; j<l ; j++)
			if (vs[i][j] == '-')
			{
				int lef = j ;		// for left ngbr
				int rig = j ;		// for right ngbr
				
				while (lef>=0 and vs[i][lef]=='-') lef--;
				while (rig<l and vs[i][rig]=='-') rig++;
				
				// for left swap {if not possible, then is infty becasue of initialisation}
				if (lef>=0) 		
				{
					int curCost = columnCost(vs,i,j,j,start,end)   + columnCost(vs,i,lef,lef,start,end);
					int finCost = columnCost(vs,i,j,lef,start,end) + columnCost(vs,i,lef,j,start,end);
				...
				if (rig<l)
				{
					int curCost = columnCost(vs,i,j,j,start,end)   + columnCost(vs,i,rig,rig,start,end);
					int finCost = columnCost(vs,i,j,rig,start,end) + columnCost(vs,i,rig,j,start,end);
